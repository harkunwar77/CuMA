...
def generate_member_permissions(emails, project_id):
    ...
    for email in email_list:
        permission = ProjectPermissions(
            user_id=email.strip(), project_id=project_id, read=True, read_write=True
        )
...
def generate_guest_permissions(emails, project_id):
    ...
    for email in email_list:
        permission = ProjectPermissions(
            user_id=email.strip(), project_id=project_id, read=True, read_write=False
        )
        permissions.append(permission)
...
def get_projects_of_user():
    try:
        ...
        combined_query = union(
            get_default_read_project_id(session), get_project_id_by_user_read(session)
        ).subquery()
        result = session.query(combined_query).all()
        project_names = [
            {"name": name, "id": project_id} for name, project_id in result
        ]
        closeSession(session)
...
def get_default_read_project_id(session):
    project_default_names = session.query(Project.name, Project.id).filter(
        Project.default_read == True
    )
...
def get_project_id_by_user_read(session):
    ...
    project_default_names = (
        ...
        .filter(
            ProjectPermissions.user_id == email_id and ProjectPermissions.read == True
        )
    )
    return project_default_names
    ...
    try:
        ...
        project_details = (
            ...
            .filter(
                Project.id == id
                and (
                    Project.default_read
                    or (
                        ProjectPermissions.read == True
                        and ProjectPermissions.user_id == getSessionUserID()
                    )
                )
            )
            .first()
        ...
        if project_details is None:
            return (
                jsonify(
                    {"error": "Project not found or user with no read permissions"}
                ),
                401,
            )
