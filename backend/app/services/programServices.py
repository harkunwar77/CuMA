...
from ..model.program import Program, ProgramOutcome
from ..model.projectProgram import ProjectProgram, ProjectProgramOutcome
...
from .projectProgramAlignmentServices import getProjectProgramAlignmentsByProgramID, deleteProjectProgramAlignments
from datetime import datetime
#function to show programs in table #
...
def addProjectProgram(project_program, outcomes):
    try:
        session = createSession()

        # Add the project program to the session
        session.add(project_program)
        session.flush()  # Commit to generate an ID for the program

        # Add outcomes for the new program
        for outcome in outcomes:
            description = outcome.get("description", "").strip()
            uga_alignment = outcome.get("uga_alignment")

            # Only add outcomes if description is provided
            if description:
                new_outcome = ProjectProgramOutcome(
                    program_id=project_program.id,
                    description=description,
                    uga_alignment=uga_alignment
                )
...
def getOfficialProgramByID(program_id):
    try:
        session = createSession()
        # Fetch the program data
        program_data = session.query(Program).filter(Program.id == program_id).first()

        if program_data is None:
            closeSession(session)
            return jsonify({"error": "Official Program not found"}), 404

        # Fetch the related outcomes for the program
        outcomes_data = session.query(ProgramOutcome).filter(ProgramOutcome.program_id == program_id).all()

        # Format outcomes to include them in the response
        outcomes = [
            {
        ...
        # Create the final program dictionary
        program = {
            "id": program_data.id,
            "name": program_data.name,
            "academic_level": program_data.academic_level,
            "faculty_id": program_data.faculty_id,
            "document_id": program_data.document_id,
            "latest_modified": program_data.latest_modified.strftime(
                "%Y-%m-%d") if program_data.latest_modified else None,
            "state": program_data.state,
            "outcomes": outcomes
        }
...
def getPLOsByProgram(project_id):
    try:
        session = createSession()
        programs = session.query(ProjectProgram).filter(ProjectProgram.project_id == project_id).all()
        
        plos = []
        for program in programs:
            for i in range(1, 21):
                description = getattr(program, f"outcome{i}_description", None)
                if description:
                    plos.append({
        ...
        return jsonify({"error": "Failed to fetch PLOs", "message": str(e)}), 500
