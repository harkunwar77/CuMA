...
from ..model.project_course import ProjectCourseOutcome
...
#function to show coursees in table #
...
# def addProjectCourse(project_course, alignments, outcome_description, outcome_alignments):
...
#             prerequisite=project_course.prerequisite,
#             postrequisite=project_course.postrequisite,
#            project_outcome=outcome_description
#         )
...
def addProjectCourse(project_course, outcomes):
    session = createSession()
    try:
        ...
        # Create and add new ProjectCourse instance
        new_project_course = Course(
            project_id=project_course.project_id,
            course_code=project_course.course_code,
            also_known_as=project_course.also_known_as,
            formerly_known_as=project_course.formerly_known_as,
            ...
            prerequisite=project_course.prerequisite,
            postrequisite=project_course.postrequisite,
            indigenous_course=project_course.indigenous_course
        )

        # Add course to session and commit to get the generated ID
        session.add(new_project_course)
        session.commit()  # Commit here to generate an ID for the course
        print(f" New Project Course ID: {new_project_course.id}")
        # Add outcomes for the new course
        for outcome in outcomes:
            print(f"Adding course outcome: {outcome}")  # Debug line to verify outcomes
            new_outcome = ProjectCourseOutcome(
        ...
        # Commit outcomes
        print("Committing the course outcomes to the database")
        session.commit()

        return jsonify({"success": "Project Course and Outcomes added successfully"}), 200
